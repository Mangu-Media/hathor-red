...
export const musicService = {
  ...
  // dose-1.90: require non-empty trimmed input (same bar as server detectMood
  // `if (!input)` 400) before POST so blank never reaches the controller.
  detectMood: (input) => {
  ...
  // dose-1.90: semantic search — server semanticSearch reads req.query.query
  // (not q); require non-empty trimmed query (same bar as `if (!searchQuery)` 400)
  // and keep limit via normalizeAiListParams (dose-1.84).
  search: (q, params) => {
    ...
    // drop legacy q if caller passed it in params
  ...
  // dose-1.90: require non-empty trimmed message (same bar as server chat
  // `if (!message)` 400) before POST.
