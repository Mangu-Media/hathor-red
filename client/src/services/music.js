...
function normalizeCreateRoom(data) {
  ...
  // dose-5.12: name length ≤ 100 parity with server roomController.createRoom
  // (dose-5.11) and middleware roomValidation so client never posts oversize.
...
function normalizeCreatePlaylist(data) {
  ...
  if (data.name == null || typeof data.name !== 'string') return null;
  const name = data.name.trim();
  // dose-5.13: name length ≤ 100 parity with server playlistController.createPlaylist
  // (dose-5.10) and middleware playlistValidation so client never posts oversize.
  if (!name || name.length > 100) return null;
  const out = { name };
  if (Object.prototype.hasOwnProperty.call(data, 'isPublic')) {
    out.isPublic = data.isPublic !== false;
