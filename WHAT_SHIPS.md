...
Last updated: 2026-09-17 (dose-5.13: client normalizeCreatePlaylist name length ≤100).
...
- **dose-5.12**: Client `normalizeCreateRoom` now rejects name length > 100 after trim (parity with server dose-5.11 / roomValidation) so Rooms create never posts an oversize name the API would 400.
- **dose-5.13**: Client `normalizeCreatePlaylist` now rejects name length > 100 after trim (parity with server dose-5.10 / playlistValidation) so Playlists create never posts an oversize name the API would 400.
...
| 5 | Olympus flags + dose-5.1–5.13 (client/server AI bars, path align, shared generateAIPlaylist, dead code retired, controller prompt + name bars, createPlaylist name/description bars, createRoom name bar, client createRoom + createPlaylist name length bars) |

## Next item

Dose 5.13 client normalizeCreatePlaylist name ≤100 complete. Prefer next thin bar on remaining mutations (e.g. description already sliced on client; updatePlaylist if present) or any residual playlist/room path; no Dose 6+.
