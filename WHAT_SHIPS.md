...
Last updated: 2026-09-16 (dose-5.6: alias /ai/playlist/generate to shared playlistController).
...
| 5 | Olympus flags + dose-5.1/5.2 client AI bars + dose-5.3 server AI controller length bars + dose-5.4 AI playlist path fix + dose-5.5 ai.js path align + dose-5.6 shared generateAIPlaylist for legacy /ai route |

## Next item

Dose 5 Olympus: OpenAI/worker missing fallbacks already honest in UI; remaining low-priority polish (e.g. retire aiController.generatePlaylist dead code path later if desired).
