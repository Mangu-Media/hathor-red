...
Last updated: 2026-09-16 (dose-5.12: client normalizeCreateRoom name length ≤100).
...
| 5 | Olympus flags + dose-5.1–5.12 (client/server AI bars, path align, shared generateAIPlaylist, dead code retired, controller prompt + name bars, createPlaylist name/description bars, createRoom name bar, client createRoom name length bar) |

## Next item

Dose 5.12 client normalizeCreateRoom name ≤100 complete. Prefer next thin bar on remaining mutations (e.g. client normalizeCreatePlaylist name length ≤100 if missing) or any residual playlist/room path; no Dose 6+.
